...
class GRAB_Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        contact_mask_gt, corr_pts_gt, corr_dist_gt = c[3][0], c[3][1], c[3][2]
        contact_mask_gt = contact_mask_gt[start_idx:start_idx+self.window_size]
        ...
        samp_ind = np.random.randint(0, self.num_points, 4000)
        ...
        object_contact_mask = data['f9'].reshape(self.window_size, -1, 1).astype(np.float32)
        ...
        contact_mask_gt = contact_mask_gt.reshape(self.window_size, -1, 1).astype(np.float32)
        ...
        contact_mask_gt[corr_dist_gt>0.1] = 0
        object_contact_mask[object_corr_dist>0.1] = 0

        window_feat = np.concatenate([object_pc, object_contact_mask, object_corr_pts,
        ...
        contact_mask_gt = contact_mask_gt[:, samp_ind[2000:]]
        ...
        if np.random.uniform() > 0.5:
            ...
            contact_mask_gt = np.flip(contact_mask_gt, axis=0).copy()
        ...
        return np.concatenate([window_feat, contact_mask_gt, corr_pts_gt, corr_dist_gt, dec_cond], axis=2)

    def __len__(self):
        return self.len


class GRAB_Dataset_Single(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        samp_ind = np.random.randint(0, 8000, 2000)
        ...
        object_contact_mask = self.data[index]['f9'].reshape(-1)[samp_ind]
        ...
        return rhand_pc, object_pc, object_contact_mask, object_corr_pts, \
